...
class DotTabLayout @JvmOverloads constructor(
        ...
        (getChildAt(0) as ViewGroup).let {
            when {
                ...
                count > it.childCount -> for (i in 0 until (count - it.childCount)) {
                    View.inflate(context, R.layout.model_item_tab_layout, it)
                }
                // Remove
        ...
        (getChildAt(0) as ViewGroup).let {
            when {
                index < it.childCount -> {
                    ...
                    Log.d("a","Unable to set tab checked at index={$index}; " +
                            "will try again once the tab views are initialized.")
                }
            }
        }
    }
    ...
    fun getThisFuckingView(wrapper: dtlWrapper){
        wrapper.tabLayout = this
    }
}

class dtlWrapper(var tabLayout: DotTabLayout? = null)
